...
#' @title rho
#' @param TH theta; all of the parameters of the model
#' @param SH sigma: all of the estimates of the variance of the mediators 
#' @param n.para number of parameters
...
#' @export The probability of the counterfactual outcome of Y
#' @keywords counterfactual
#' @examples rho(theta.hat,sig.hat,n.para,Name=c("W","Q","S"),n.cate1,a,w0000,confounders)

rho<-function(TH,SH,n.para,Name=c("W","Q","S"),n.cate1,a,w,confounders=NULL){
  a0<-TH[[2]][1]
  aw<-TH[[2]][2]
  aq<-TH[[2]][3]
  d0<-TH[[3]][1]
  ...
  prhobC <- rep(0,n.C*n.cate1)
  prhob0 <- rep(0,n.cate1); prhobW <- rep(0,n.cate1)
  prhobQ <- rep(0,n.cate1); prhobS <- rep(0,n.cate1)
  ...
  for(aa in 1:n.cate1){
    if(aa==(a-1)){
      for(k in 1:n.C){
        prhobC[(k-1)*n.cate1+(a-1)] <- pb0_1*confounders[k]
      }
      prhob0[(a-1)] <- pb0_1; prhobW[(a-1)] <- pbw_1
      prhobQ[(a-1)] <- pbq_1; prhobS[(a-1)] <- pbs_1
    }else if(aa==a){
      for(k in 1:n.C){
        prhobC[(k-1)*n.cate1+(a)] <- pb0*confounders[k]
      }
